...
class TM_FireCheckout_IndexController extends Mage_Checkout_OnepageController
{
    ...
    {
        return $this->getUpdateCheckoutLayout()->getBlock('checkout.coupon')->toHtml();
    }
    ...
    {
        return $this->getUpdateCheckoutLayout()->getBlock('checkout_cart_coupon_normal')->toHtml();
    }
                ...
                if ($result['message'] === $checkout->getCustomerEmailExistsMessage()) {
                    ...
                    $result['body'] = array(
                        ...
                        'content' => $this->getLayout()->createBlock('core/template')
                            ->setTemplate('firecheckout/emailexists.phtml')
                            ->toHtml()
                    );
                } else {
                    $result['error_messages'] = $result['message'];
...
//                            ->setTemplate('firecheckout/sagepay/iframe.phtml')
//                            ->toHtml()
//                    )
//                )));
//            }
            ...
            // billsafe
            if (0 === strpos($paymentData['method'], 'billsafe')) {
                $quote->save();
                return $this->getResponse()
                    ->setBody(Mage::helper('core')->jsonEncode(array(
                        'method' => $paymentData['method']
