...
class Mage_Catalog_Block_Navigation extends Mage_Core_Block_Template
{
        ...
        $isOutermost = false, $outermostItemClass = '', $childrenWrapClass = '', $noEventAttributes = false)
    {
        ...
        $html[] = '<span class="navigation-name">' . $this->escapeHtml($category->getName()) . '</span>';
         if ($hasActiveChildren && $level == 0 && (!in_array('last', $classes))) $html[] = '<span class="arrow">&nbsp;</span>';
        $html[] = '</a>';
        ...
        if (!empty($htmlChildren)) {
            ...
            $html[] = $htmlChildren;
            $html[] = '</ul>';
            if(!in_array('last', $classes)) $html[] = $this->getFeaturedProductsBox($categorytmp);
            if ($childrenWrapClass) {
    ...
    public function getFeaturedProductsBox($category) {
        ...
        $featured_products = $category->getProductCollection()
            ->addAttributeToSelect('*')
            ->addFieldToFilter(
                array(
                    array(
                        'attribute'=>'max_featured','eq'=>'1'
                    ),
                )
            )->load();
