        ...
        if (strlen(trim($xmlData)) > 0) {
            ...
            $parsedOrder = array(
                ...
                'shippingAddress'   => array(
                    ...
                    'street2'        => (string) $xml->descend("ProcessedOrder/ShippingAddress/AddressFieldTwo"),
                    ...
                    'telephone'     => (string) $xml->descend("ProcessedOrder/ShippingAddress/PhoneNumber"),
                ),
                'items'             => array(),
            );

Mage::log($parsedOrder, null, 'amazon-parse-order.log');
...
Mage::log($xmlData, null, 'amazon-parse-order-ready-to-ship.log');

        $readyToShipData = array();
        if (strlen(trim($xmlData))) {
            $xml = simplexml_load_string($xmlData, 'Varien_Simplexml_Element');
            $aOrderId = (string) $xml->descend('ProcessedOrder/AmazonOrderID');
            $readyToShipData['telephone']  =  (string) $xml->descend("ProcessedOrder/ShippingAddress/PhoneNumber");
...
Mage::log($readyToShipData, null, 'amazon-parse-order-ready-to-ship.log');
    ...
    protected function _parseRequestAddress($xmlResponse)
    {
        ...
        if (strlen(trim($xmlResponse)) > 0) {
            ...
            $address = array(
                ...
                'street2'            => (string) $xml->descend("CallbackOrders/CallbackOrder/Address/AddressFieldTwo"), // wdb
