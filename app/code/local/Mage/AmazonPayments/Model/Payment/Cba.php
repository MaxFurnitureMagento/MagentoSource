...
Mage::log($_request, null, 'amazon-request.log');
...
//                     $orderReadyToShipDetails = $this->getApi()->parseOrder($_request['NotificationData']);
                    $orderReadyToShipDetails = $this->getApi()->parseOrderReadyToShipNotification($_request['NotificationData']);
...
Mage::log($newOrderDetails, null, 'amazon-new.log');
...
Mage::log($quote->getData(), null, 'amazon-quote.log');
...
//         $sLastname = isset($sName[1])?$sName[1]:'';
				...
				$temp_name = trim($newOrderDetails['shippingAddress']['name']);
				preg_match("/^([^\s$]*)\s*([^\s$]*)\s*(.*)/", $temp_name, $rx);

				if(isset($rx[3]) && !empty($rx[3]))
					{
						$sLastname = $rx[3];
						$sFirstname = $rx[1].' '.$rx[2];
					}
				elseif(isset($rx[2]) && !empty($rx[2]))
					{
						$sLastname = $rx[2];
						$sFirstname = $rx[1];
					}
...
//         $bLastname = isset($bName[1])?$bName[1]:'';
				...
				$temp_name = trim($newOrderDetails['buyerName']);
				preg_match("/^([^\s$]*)\s*([^\s$]*)\s*(.*)/", $temp_name, $rx);

				if(isset($rx[3]) && !empty($rx[3]))
					{
						$bLastname = $rx[3];
						$bFirstname = $rx[1].' '.$rx[2];
					}
				elseif(isset($rx[2]) && !empty($rx[2]))
					{
						$bLastname = $rx[2];
...
//-------------------------------------------------------------------
        ...
        $shipping->setCountryId($_address['countryCode'])
            ...
            ->setStreet(array($_address['street'], $_address['street2']))
            ...
            ->setLastname($sLastname)
						->setEmail($newOrderDetails['buyerEmailAddress']);
...
// 				$quote->getCheckoutMethod('guest');

        $billing = $quote->getBillingAddress();
        $shipping = $quote->getShippingAddress();

Mage::log("order", null, 'amazon-order.log');
Mage::log($billing->getData(), null, 'amazon-billing.log');
Mage::log($shipping->getData(), null, 'amazon-shipping.log');
...
Mage::log($customer, null, 'amazon-customer.log');
        if ($customer && $customer->getId()) {

Mage::log("usao", null, 'amazon-uslov.log');
...
Mage::log("nije usao", null, 'amazon-uslov.log');
...
Mage::log($order->getData(), null, 'amazon-order.log');
                    ...
                    if (array_key_exists($item->getExtOrderItemId(), $amazonOrderDetails['items'])) {
                        $item->setLockedDoInvoice(false)
                            ...
                            ->save();
								...
								$billing->setTelephone($amazonOrderDetails['telephone']);
								$shipping->setTelephone($amazonOrderDetails['telephone']);

Mage::log($amazonOrderDetails, null, 'amazon-process-order.log');
...
//echo 'sds';print_r($_quote->getData());die;
