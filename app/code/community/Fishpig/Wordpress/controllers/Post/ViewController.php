...
class Fishpig_Wordpress_Post_ViewController extends Fishpig_Wordpress_Controller_Abstract
{
	...
	protected function _init()
	{
		...
		$this->_useOpenGraphTags = Mage::helper('wordpress/opengraph')->isEnabledForPosts();
		...
		if ($post = $this->_getPost()) {
			...
			if ($this->_useOpenGraphTags) {
				Mage::helper('wordpress/opengraph')->addTag('url', $post->getUrl())->addTag('type', 'article');
				
				if ($image = $post->getFeaturedImage()) {
					Mage::helper('wordpress/opengraph')->addTag('image', ($image->getLargeImage() ? $image->getLargeImage() : $image->getMediumImage()));
				}
			}
