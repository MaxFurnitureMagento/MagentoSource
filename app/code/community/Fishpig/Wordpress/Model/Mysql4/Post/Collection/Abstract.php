...
abstract class Fishpig_Wordpress_Model_Mysql4_Post_Collection_Abstract extends Mage_Core_Model_Mysql4_Collection_Abstract
{
	...
	public function joinTermTables($type)
	{
		...
		if (!isset($this->_termTablesJoined[$type])) {
			...
			$this->getSelect()->join(array('tax_' . $type => $tableTax), "`tax_{$type}`.`term_taxonomy_id`=`rel_{$type}`.`term_taxonomy_id` AND `tax_{$type}`.`taxonomy`='{$type}'", '');
			$this->getSelect()->join(array('terms_' . $type => $tableTerms), "`terms_{$type}`.`term_id` = `tax_{$type}`.`term_id`", '');
