...
class Celebros_Conversionpro_Model_ObserverLarge extends Celebros_Conversionpro_Model_Exporter
{
	...
	public function export_celebros($webAdmin) {
		...
		foreach (Mage::app()->getStores() as $store) {
			...
			$this->_fileNameZip=Mage::getStoreConfig('conversionpro/export_settings/zipname',$this->_fStore_id);
	...
	protected function export_extra_tables()
	{
		...
		$extraTablesData=Mage::getStoreConfig('conversionpro/export_settings/extra_tables',0);
		$extraTables=explode("\n",$extraTablesData);
	...
	protected function export_tables() {
		...
		$table = $this->getTableName("conversionpro_mapping");
		$this->logProfiler("START {$table}");
		$this->logProfiler('Mem usage: '.memory_get_usage(true));
		$query = $read->select()
			->from($table,
		...
		$this->export_table($query, "conversionpro_mapping");
		$this->logProfiler("FINISH {$table}");
		$this->logProfiler('Mem usage: '.memory_get_usage(true),true);		
	...
	protected function export_products($sql, $fileName)
	{
		...
		$fields = array('id', 'price', 'image_link', 'thumbnail', 'type_id', 'sku', 'is_saleable', 'is_in_stock', 'qty', 'min_qty');

		if($this->bExportProductLink) $fields[] = 'link';

		foreach ($fields as $key => $field) {
			$fields[$key] = Mage::helper('conversionpro/mapping')->getMapping($field);
		}
		...
		Mage::getModel('conversionpro/cache')->setName('export_custom_fields')->setContent(json_encode($customAttributes))->save();
		
		//Dispatching event in case a custom module would want to modify the export process.
		Mage::dispatchEvent('conversionpro_before_export_products', array(
                'fields'             => &$fields,
				'sql'				 => &$sql,
				'filename'			 => &$fileName
            ));
		...
		$str='';
		$rows=$stm->fetchAll();
		$chunks = array_chunk($rows, Mage::helper('conversionpro')->getExportChunkSize());
		$i = 1;
		$pids = array();
		$finished = array();
		
		foreach ($chunks as $chunk) {
			if (count($pids) >= Mage::helper('conversionpro')->getExportProcessLimit()) {
				$counter = 10;
				do {
					$counter--;
					if ($counter == 0) break;
			...
			Mage::getModel('conversionpro/cache')->setName('export_chunk_'.$i)->setContent(json_encode($chunk))->save();
			$pids[$i] = (int)shell_exec('nohup php ' . Mage::getModuleDir('', 'Celebros_Conversionpro') . '/Model/processChunkCol.php '.$i.' '.$this->_fStore_id.' > /dev/null & echo $!');
			if (!$pids[$i]) {
		...
		$_fPath = Mage::app()->getStore(0)->getConfig('conversionpro/export_settings/path').'/'.$this->_fStore->getWebsite()->getCode().'/'.$this->_fStore->getCode();
		...
		Mage::getModel('conversionpro/cache')->getCollection()
