...
class Celebros_Conversionpro_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    {
		//Changed this from getFacetedData to the other option, but there
		// might have been a reason why I wanted it that way in the first
		// place, so take in mind that we might want to change it back.
    	
    	//$key = Mage::getResourceSingleton('conversionpro/fulltext_engine')
		//	->getCelebrosSearchEngineQuestionFieldName($fieldName);
	...
	public function registerImageBanner($link) {
     ...
     * @return  bool
	 *
	 * @todo we can remove this function and use getIsEngineAvailable() in its place.
	 * In that case, we'll be able to unify resetCurrentCatalogLayer() and resetCurrentSearchLayer() as well.
     */
    public function getIsEngineAvailableForNavigation($isCatalog = true)
    {
		...
		if (is_null($this->_isEngineAvailableForNavigation)) {
			...
			if ($this->isActiveEngine()) {
                ...
                //If this is a search results page, mark conversionpro as available.
				} else {
                    $this->_isEngineAvailableForNavigation = true;
                }
            }
	...
	/*
	 * This function is the same as getIsEngineAvailableForNavigation(), only it doesn't require you to know in advance
	 * whether you're on a catalog or a search page.
	 */
     ...
     *
	 * @todo there's old code here that has to do with the way we used to enable/disable the plugin, via catalog/search/engine.
	 * we're now using a different method so we can remove the commented section.
	 *
     * @return bool
     */
    public function isActiveEngine()
    {
		...
		/*
		//Check the engine that's set on Catalog->Catalog Search->Search Engine.
		$engine = $this->getSearchConfigData('engine');
        
        if ($engine && $engine == 'conversionpro/fulltext_engine' && Mage::getConfig()->getResourceModelClassName($engine)) {
            $model = Mage::getResourceSingleton($engine);
            if ($model->allowAdvancedIndex()) {
                return true;
            }
        }
		*/
     ...
     *
	 * @todo check if we can remove this function, it might not really be in use.
	 *
     * @return bool
     *
    ...
    {
        $engine = $this->getSearchConfigData('engine');
        if ($engine == 'conversionpro/fulltext_engine') {
            return true;
        }
    ...
    */
     ...
     * @return string|int
	 *
	 * @todo check if this could be removed, now that we're not using catalog/search/engine to enable/disable the plugin.
     */
	...
	/**
	 * @todo replace getStoreConfigFlag() with getCelebrosConfigData()
	 *
	 */
	public function isCampaignsEnabled()
	{
		return Mage::getStoreConfigFlag('conversionpro/display_settings/campaigns_enabled');
	}
	...
	{
		return Mage::getStoreConfigFlag('conversionpro/general_settings/conversionpro_enabled');
	}
	...
	{
		return (Mage::getStoreConfigFlag('conversionpro/livesight_settings/livesight_enabled')
				&& $this->isConversionproEnabled());
	}
	
	public function getProfileName()
	{
		return Mage::getStoreConfig('conversionpro/display_settings/profile_name');
	}
	...
	public function getQuestionByAttributeCode($att_code) {
    	...
    	$key = $this->getSearchEngine()->getCelebrosSearchEngineQuestionFieldName($att_code);
	...
	public function refreshQuestions()
	{
		...
		foreach($orig_questions as $question) {
			if ($question->Id == 'PriceQuestion') {
				$questions[$searchResults->SearchInformation->PriceFieldName] = $question;
			} else {
			...
			if ($question->Id == 'PriceQuestion') {
				$this->addQuestionText($this->getSearchEngine()->getPriceFieldName());
			} else {
	...
	public function addQuestionText($questionText)
	{
		...
		if (!array_key_exists(strtolower($questionText), $this->_questionTexts)) {
			$this->_questionTexts[strtolower($questionText)] = $questionText;
	...
	public function getAnswerIdByCategoryId($category_id)
	{
		$this->_answers = $this->getCategoryMapping();
		...
		//If this didn't work, run it again but get the list of answer ids from conversionpro first.
		$this->_answers = $this->resetCategoryMapping();
		foreach ($this->_answers as $answerId => $categoryId) {
			if ($categoryId == $category_id) {
				return $answerId;
	...
	public function getAnswersMapping($att_code)
	{
		...
		if (!is_array($this->_answersMapping) || !array_key_exists($key, $this->_answersMapping)) {
			$this->_answersMapping = $this->resetAnswersMapping();
		}
		return $this->_answersMapping[$key];
	}
	...
	public function resetAnswersMapping()
	{
		...
		if (isset($salespersonApi->results)) {
			...
			foreach ($salespersonApi->results->Items as $question) {
				...
				if ($question->Id == 'PriceQuestion') {
					$this->_answersMapping[$this->getSearchEngine()->getPriceFieldName()] = $answers;
				} else {
