 ...
 */
class Celebros_Conversionpro_Block_System_Convert_Profile_Edit_Tab_Run extends Mage_Adminhtml_Block_System_Convert_Profile_Edit_Tab_Run
{
    public function getRunButtonHtml()
    {
        $profile_name=Mage::registry('current_convert_profile')->getName();   
        $html = '';
        if ($profile_name == "Conversion Pro Exporter")
        {
            $export_URL = Mage::helper("adminhtml")->getUrl
                ("conversionpro_admin/export/export_celebros/webadmin/true");

            $schedule_URL = Mage::helper("adminhtml")->getUrl
                ("conversionpro_admin/export/schedule_export");


            $html .= $this->getLayout()->createBlock('adminhtml/widget_button')->setType('button')
                ->setLabel($this->__('Export Products'))
                ->setOnClick('window.open(\''.$export_URL.'\')')
                ->toHtml();

            $html .= '<br/><br/>';
            $html .= $this->getLayout()->createBlock('adminhtml/widget_button')->setType('button')
            ->setLabel($this->__('Schedule Cron Export'))
