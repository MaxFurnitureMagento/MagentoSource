...
(function($) {
	$.fn.ajaxZoomSlider.dynOptions  = function(){
		...
		// This is a map of all possible values used for dynamic configuration in http://www.ajax-zoom.com/examples/example30.php
		var defVars = {
			parameter: {
				values: 'textarea', 
				newBlock: 'Most Important (please read)', 
				subBlock: 'Because of sucurity issues the size of the player can only be edited by setting \
				AJAX-ZOOM option <code>$zoom[\'config\'][\'picDim\']</code> in /axZm/zoomConfigCustom.inc.php after <code>elseif ($_GET[\'example\'] == \'imageSlider\'){</code>',
				...
				Btw: here <a href="http://www.ajax-zoom.com/examples/example27.php">http://www.ajax-zoom.com/examples/example27.php</a> is a good overview of some \
				extended possibilities to define what you want to see in this image slider :-) \
				' 
				
			},
			axZmPath: {values: 'string', newBlock: 'Necessary Options', descr: 'Path to AJAX-ZOOM - /axZm/ directory, e.g. "/apps/zoom/ajax-zoom/axZm/"'},
			...
			bulletsContainerMargin: {values: 'int', descr: 'Margin of the container with bullets'},
			...
			playPauseInterval: {values: 'int', descr: 'Frame interval (time in ms between image change)'},
...
<pre style="font: 9pt arial; background-color: #FFFFCC; padding: 5px">descriptionObject: {\n \
	"example_image_1.jpg": {\n \
...
As of first version of this slider there is no comfortable configurator available. <br /><br />\
			...
			fluidDesign: {values: [true, false], descr: 'Use this option when the size of divID - the container, where AJAX-ZOOM is placed into, can vary depending on users screen resolution. Do not activate it if divID has fixed with and height.'},
		...
		function colorizeObj(json){
			...
			json = strReplace('}\n', '<span style="color: blue;">}</span>\n', json);
			return json;
			//return json.replace(/:/, '\"<span style="color: blue;">:</span>')
		}
		...
		var printOptions = function(){
			...
			$.each(defaults, function(opt, val){
				if (defVars[opt]){
					var custom = $('#'+opt).val();
					//var custom = $("input[name='"+opt+"']").val();
					if (custom == 'false'){custom = false;} else if (custom == 'true'){custom = true;} else if (isInt(custom)){custom = parseInt(custom);}
			...
			$.each(customOpt, function (k, v){
				...
				if (numCustomOpt != n){customOptString += ',\n'}
			});
			
			$('#axZmSliderDynConfPrint').html('<pre>jQuery.fn.ajaxZoomSlider<span style="color: blue">({</span>\n'+customOptString+'\n<span style="color: blue">})</span>;</pre>');
		}		
		...
		function createSelect(name, arr, selected){
			var r = $('<select />').attr({
				id: name,
				name: name
			}).css({width: colW[2]-20}).bind('change', printOptions);
		...
		function createTextField(name, value, width){
			...
			var r = $('<input>').attr({
				...
				name: name,
				id: name
			}).css({width: width}).bind('change', printOptions);
			return r;
		}
		...
		function createTextArea(name, value, width, height){
			var r = $('<textarea />').attr({
				...
				name: name,
				id: name
			}).css({width: width, height: height}).bind('change', printOptions);
			return r;			
		}
		...
		var submitOptionsForm = function(event){
			...
			var r = {};
			$('#axZmSliderDynConf :input').each(function(){
				
				var f = $(this); var name = f.attr('id'); var val = f.val();
				var brk = false;
				if (val == 'false'){val = false;} 
				else if (val == 'true'){val = true;} 
				else if (isInt(val)){val = parseInt(val);}
			...
			$('#axZmSliderDynConf').toggle(); $('#axZmPlayerOuter').toggle();
			$('#'+loadedOptions.divID).empty().html('<div style="margin: 10px; font-size: 150%">Reloading with new options, please wait</div>');
			$('#zFsO').remove();

			setTimeout(function(){$.fn.ajaxZoomSlider(loadedOptions);}, 500);
			window.scrollTo(0,0);
		...
		var color = '#000000';
		var colW = {1: 200, 2: 180, 3: 80, 4: 320};
		colW[4] = $('#axZmSliderDynConf').width() - colW[1] - colW[2] - colW[3] - 20;
		
		// Heading
		var newDivCont = $('<div />').css({float: 'left', clear: 'both', width: '100%', padding: 3, marginTop: 20, height: 22, fontSize: '125%', cursor: 'pointer', backgroundColor: '#2379B5', color: '#FFFFFF'});
		var optNameDiv = $('<div />').css({float: 'left', width: colW[1]}).html('Option').appendTo(newDivCont);

		var optSelDiv = $('<div />').css({float: 'left', width: colW[2]}).html('Values').appendTo(newDivCont);
		var optDefault = $('<div />').css({float: 'left', width: colW[3]}).html('Default').appendTo(newDivCont);
		
		var optDescDiv = $('<div />').css({float: 'right', height: 'auto', width: colW[4]}).html('Description').appendTo(newDivCont);			
		
		newDivCont.bind('click', function(){
			$("[id*=sliderDynConfSubDiv]").slideToggle();
		})
		...
		$.each(defaults, function(opt, val){
			...
			if (isObject(defVars[opt])){
				...
				if (defVars[opt]['newBlock']){
					...
					// Submit button
					var button = $('<div />').css({float: 'right', width: 150, color: '#000000', height: 16, top: 0, padding: 1, paddingLeft: 5, borderColor: '#FFFFFF'})
					.addClass('axZmPlayerDynOptionsButton')
					.html('SUBMIT NEW OPTIONS').bind('click', submitOptionsForm);		 
					
					// Subheading
					$('<div />').css({float: 'left', clear: 'both', marginTop: 5, color: '#FFFFFF', padding: 3, height: 20, width: '100%', backgroundColor: '#555555'})
					.html('<span style="font-size: 120%">'+defVars[opt]['newBlock']+'</span>')
					.append(button)
					.bind('click', function(){$(this).next().slideToggle();})
					.appendTo('#axZmSliderDynConf');
					
					// Subcontainer
					var subDiv = $('<div />').attr('id', 'sliderDynConfSubDiv'+mn)
					.css({float: 'left', clear: 'both', width: '100%', height: 'auto', display: 'none'}).appendTo('#axZmSliderDynConf');
					
					if (defVars[opt]['subBlock']){
						$('<div />').css({float: 'left', clear: 'both', marginTop: 5, marginBottom: 5, color: '#000000', padding: 3, height: 'auto', width: '100%', backgroundColor: '#FFFFCC'})
				...
				var newDivCont = $('<div />').css({float: 'left', clear: 'both', padding: 3, width: '100%', backgroundColor: backgroundColor, color: color});
				var optNameDiv = $('<div />').css({float: 'left', width: colW[1]}).appendTo(newDivCont);
				...
				if (defVars[opt]['values'] == 'textarea'){
					optSelDiv.css({float: 'left', width: colW[2] + colW[3]}).appendTo(newDivCont);
					optDescDiv.css({float: 'right', height: 'auto', width: colW[4]}).appendTo(newDivCont);
				} else if (defVars[opt]['values'] == 'longDescription'){
					optDescDiv.css({float: 'left', width: colW[2] + colW[3] + colW[4]}).appendTo(newDivCont);
				} else {
					optSelDiv.css({float: 'left', width: colW[2]}).appendTo(newDivCont);
					optDefault.css({float: 'left', width: colW[3]}).appendTo(newDivCont);
					optDescDiv.css({float: 'right', height: 'auto', width: colW[4]}).appendTo(newDivCont);		
				}
				...
				if ($.isArray(defVars[opt]['values'])){
					...
					sel = createTextField(opt, val, 70);
				} else if (defVars[opt]['values'] == 'string'){
					sel = createTextField(opt, val, 150);
				} else if (defVars[opt]['values'] == 'textarea'){
					sel = createTextArea(opt, val, colW[2] + colW[3], 240);
				} 
				...
				if (defVars[opt]['values'] != 'textarea' && !noDefault){
					optDefault.append(string(val, true));
				}
		...
		//$('<div />').css({clear: 'both', width: 150, height: 18}).addClass('axZmPlayerDynOptionsButton')
		//.html('SUBMIT NEW OPTIONS').bind('click', submitOptionsForm).appendTo('#axZmSliderDynConf');
	...
	$(document).ready(function(){
		...
		setTimeout(function(){$('#axZmSliderDynConf').toggle()}, 200);
	});
