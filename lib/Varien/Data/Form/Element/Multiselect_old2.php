...
//                $this->serialize($this->getHtmlAttributes()) . ' multiple="multiple">' . "\n";
        ?>
            ...
            <script type="text/javascript" src="/js/prototype/jquery-1.4.4.min.js"></script>
            <script type="text/javascript">             
                jQuery.noConflict();
                jQuery(document).ready(function(){
                        var regExp = /catalog_product/;
                        var urlString = window.location.href;
                        var result = regExp.test(urlString);
                        //alert(result);
                        if(result)                 
                        {                
                                ...
                                jQuery('select[multiple="multiple"]').each(function(){
                                    var selected_options = jQuery(this).find('option:selected');
                                    var return_value = "";
                                    selected_options.each(function(){
                                        return_value = return_value + jQuery(this).val()+",";
                                    });
            //                        console.log(selected_options);
                                    jQuery(this).parent().find('input[name="<?php echo parent::getName(); ?>"]').val(return_value);
                                });
                                jQuery('select[multiple="multiple"]').on('change',function(){
                                    var selected_options = jQuery(this).find('option:selected');
                                    var return_value = "";
                                    selected_options.each(function(){
                                        return_value = return_value + jQuery(this).val()+",";
