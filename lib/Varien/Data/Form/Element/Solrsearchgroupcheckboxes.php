 ...
 */
class Varien_Data_Form_Element_SolrSearchGroupCheckboxes extends Varien_Data_Form_Element_Abstract
{
    protected $tempHtmlName = "";
    /**
     * Init Element
     *
    ...
    public function getElementHtml()
    {
        	...
        	foreach ($value1['value'] as $value2) {
				...
				$html.= '<ul style="padding-left:20px">';
					if(count($value2['value'])){
						...
						$html.= '<li>There is no store view available.</li>';
					}
				$html.= '</ul>';
				$html.= '</li>';
        	}
        ...
        $html .= '<input type="checkbox" style="display:none" checked="checked" name="'.$this->tempHtmlName.'" value="," />';
        $html .= '</ul>'
...
//    }
    ...
    protected function _optionToHtml($option)
    {
		...
		$id = $this->getHtmlId().'_'.$this->_escape($option['value'][0]['value']);
		}
