 ...
 * @category   Sohi
 * @package    Sohi_Core
 * @copyright  Copyright (c) 2013 Piotr Socha
...
//                 self::$_filePath = BP;
//             } else {
                self::$_filePath = dirname(dirname(dirname(__FILE__)));
//             }
        }
...
//     }
	...
	/*
        ...
        $file = empty($file) ? date('Y_m_d_H') . '_sohi.log' : $file;

        try {
            if (!isset($loggers[$file])) {
                $logDir  = self::getRootPath() . DS . 'var' . DS . 'log';
