    ...
    save: function(urlSuffix, forceSave) {
        ...
        for (var i=0; i<shippingMethods.length; i++) {
            if (shippingMethods[i].checked) {
                activeShippingMethod = shippingMethods[i].value;
            }
        }
        if (!forceSave && activeShippingMethod.indexOf("tnt_") === 0) {
...
Billing.prototype = {
    initialize: function(){
        ...
        if (FireCheckout.prototype.ajax.total_on_taxvat) {
            $('billing:taxvat') && $('billing:taxvat').observe('change', function() {
